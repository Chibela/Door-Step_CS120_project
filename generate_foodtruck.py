...
<head>
    <meta charset="UTF-8">
    <link rel="icon" type="image/png" href="Images/FoodTruck.png" />
...
<body>
    ...
    <div class="cart" id="cart">
        ...
        <div class="cart-content">
            ...
            <div class="cart-breakdown" id="cartBreakdown" style="display: none;">
    ...
    <script>
        ...
        function updateCart() {{
            ...
            if (cart.length === 0) {{
                cartItems.innerHTML = '<p style="text-align: center; color: #999; padding: 1rem;">Your cart is empty</p>';
            ...
            document.getElementById('tipAmount').textContent = `$${{selectedTip.toFixed(2)}}`;
        ...
        function selectTip(amount) {{
            ...
            document.querySelectorAll('.tip-btn').forEach((btn, index) => {{
                btn.classList.remove('active');
                const tipValues = [0, 10, 15, 20];
                if (tipValues[index] === amount) {{
        ...
        function removeItem(itemId) {{
            ...
            // Show feedback
            const itemName = document.querySelector(`[onclick*="removeItem('${{itemId}}')"]`)?.closest('.cart-item')?.querySelector('h4')?.textContent || 'Item';
        ...
        function decreaseQuantity(itemId) {{
            ...
            // Edge case: Quantity reaches 0, auto-remove
            if (item.quantity <= 0) {{
                removeItem(itemId);
                return;
            }}
        ...
        function checkout() {{
            ...
            const subtotal = cart.reduce((sum, item) => sum + (item.price * item.quantity), 0);
            const tax = subtotal * TAX_RATE;
            const finalTotal = subtotal + tax + selectedTip;
            ...
            showAlert(`Order placed successfully! Total: $${{finalTotal.toFixed(2)}} (including $${{tax.toFixed(2)}} tax + $${{selectedTip.toFixed(2)}} tip)`, 'success');
        }}
